(function() {
  ...
  function CanvasCtrl($scope,
    ...
    vm.onItemDelete = function(observation) {
      var observationIndex = carDamagesService
        .damages
        .indexOf(observation);
      deleteShape(observation.shapeId);
      carDamagesService.damages.splice(observationIndex, 1);
    };
    ...
    function addDamagesToCanvas(damagesList) {
      for (var position = 0; position < damagesList.length; position++) {
        ...
        var isDamageNew = (carDamagesService.damagesLoaded || damage.is_new) ? true : false;
        ...
        carDamagesService.addDamageToCanvasComponents(canvasItem);
      }
      paper.view.update();
      carDamagesService.damagesLoaded = true;
    }
    ...
    function setVehicleBackground() {
      var raster = null;
      var tractionType = carInfoService.carInfo.traction_type;
